...
export function ChangeEmploymentModal({ selectStudent, setSelectStudent, refetch }: PropType) {
	...
	const { data: keywordList, isLoading, refetch: refetchKeywordList } = useGetCode('TECH');
	const filteredList = keywordList?.codes.filter((keywords) => keywords.keyword.includes(searchInput.searchInputValue));
	const dataLength = (filteredList?.length || 0) / 2;

	const changeEmploymentAPI = useChangeEmployment(selectCodeKeywords, selectStudent, {
		onSuccess: () => {
			refetch();
			setSelectStudent([]);
		...
		.flatMap((keywords, index) => {
			const clickCheckBox = (keywordName: string) => {
				if (selectCodeKeywords.includes(keywordName)) {
					setSelectCodeKeywords((keywords) => keywords.filter((keyword) => keyword !== keywordName));
				} else {
					setSelectCodeKeywords((keywords) => [...keywords, keywordName]);
				}
			};
			...
			if (index % 2 === 0) {
				if (index < filteredList.length - 1) {
					...
					return [
						[
							<CheckBox onChange={() => clickCheckBox(keywords.keyword)} checked={selectCodeKeywords.includes(keywords.keyword)} />,
							<_.ContentText>{keywords.keyword}</_.ContentText>,
							<CheckBox onChange={() => clickCheckBox(nextValue.keyword)} checked={selectCodeKeywords.includes(nextValue.keyword)} />,
	...
	const tableTitle: JSX.Element[] = [<CheckBox disabled={true} />, <_.TitleText>이름</_.TitleText>, <CheckBox disabled={true} />, <_.TitleText>이름</_.TitleText>];
	...
	return (
		<_.Container>
			...
			<Pagination page={Math.max(Math.floor((filteredList?.length || 0) / 8), 1)} data={pages} setData={setPages} refetch={() => {}} />
		</_.Container>
	);
}
